...
test('TestSuite_03_VO_Regression_CreateItems', async ({ page }) => {
    ...
    const EquipItem =  testData('CreateItems','TC_05_VO_Regression_CreateAEquipment','EquipItem')
    ...
    await test.step("TC_01_VO_Regression_CreateAConsumItem", async()=>
        {
          await Item.itemCreation("consumable",ItemPart1+rand,ItemDesc1,ItemManf1,ItemCategory1,ItemSupp1,ItemCost1,ItemPrice1,ItemTonerType1)
                    
        })

    await test.step("TC_02_VO_Regression_CreateAPart", async()=>
        {
          await Item.itemCreation("part",ItemPart2+rand,ItemDesc2,ItemManf2,ItemCategory2,ItemSupp2,ItemCost2,ItemPrice2,ItemTonerType2)
    ...
    await test.step("TC_03_VO_Regression_CreateAModel", async()=>
        {
          ...
          await Item.itemCreation("model",ItemPart3+rand,ItemDesc3,ItemManf3,ItemCategory3,ItemSupp3,ItemCost3,ItemPrice3,ItemTonerType3,ItemMeterPreSet1_3 ,ItemMeterPreSet2_3)
          await page.waitForTimeout(5000)
    ...
    await test.step("TC_04_VO_Regression_CreateAAccessory", async()=>
        {
          ...
          await Item.itemCreation("accessory",ItemPart4+rand,ItemDesc4,ItemManf4,ItemCategory4,ItemSupp4,ItemCost4,ItemPrice4)
    ...
    await test.step("TC_05_VO_Regression_CreateAEquipment", async()=>
        {
          ...
          await Equipment.equipmentCreation(EquipItem,EquipSerialNo)
          await page.waitForTimeout(2000)
